...
- Commit: `8ce28a6 feat: 14-stage pipeline with dynamic MCQ questions + pertanyaan_mobile stage`
- Status: [x] Selesai.

### 2026-08-08 · GlitchTip Disabled + Docker Healthchecks & Dependencies (ops)
- Dikerjakan: (1) Tambah healthcheck di semua service (`aiplanstudio_apifpm`, `aiplanstudio_web`, `aiplanstudio_redis`, `glitchtip`, `nginx`) + ubah `depends_on` ke `condition: service_healthy` semuanya (bukan `service_started`). Fix Next.js hanya bind ke container IP → tambah `HOSTNAME=0.0.0.0`; healthcheck pakai `127.0.0.1` (localhost resolve IPv6 `::1`). Fix api-fpm healthcheck `nc -z 9000`; GlitchTip healthcheck via python3 `/_health/` (tak ada wget). **Semua container kini healthy.** (3) **Matikan GlitchTip**: service di-comment di docker-compose.yml; route `location /glitchtip/` di nginx di-comment; DSN `SENTRY_*`+`GLITCHTIP_*` di .env/.env.example di-comment; build arg `NEXT_PUBLIC_SENTRY_DSN` di Dockerfile & compose web di-comment; image `glitchtip/glitchtip:6` dihapus. SDK (`sentry/sentry-laravel`, `@sentry/nextjs`) **dipertahankan** sebagai no-op (DSN kosong → enabled: false).
- Perintah/hasil: `docker compose config --quiet` → OK. `docker compose up -d` → 6 container healthy. `curl localhost:4197/` → 200. `docker rmi glitchtip/glitchtip:6` → deleted.
- Kendala: bind web HTTPS dulu error di healthcheck `localhost` (IPv6); api-fpm `/ping` tak ada; glitchtip tanpa wget — semua diperbaiki (127.0.0.1, nc, python3).
- Status: [x] Selesai. GlitchTip dapat diaktifkan kembali (lihat catatan di docker-compose.yml).

### 2026-08-08 · Pipeline 13-Stage Overhaul — split phased_master, hapus target mobile, gate
- Dikerjakan: (1) Split `phased_master` (1 stage → 4 artifact) menjadi 4 stage terpisah: `standards_web`, `agents_web`, `phases_web`, `master_web`. Split `phased_master_mobile` → `phases_mobile`, `standards_mobile`, `agents_mobile`, `master_mobile`. Total 13 stage (both) / 9 stage (web). (2) Hapus target `mobile` standalone — hanya `aiplanstudio_web` dan `both`. (3) Gate: mobile track menunggu `master_web` done. (4) Update PipelineRunner: ALL_STAGES 13, MOBILE_STAGES, WEB_DONE_STAGE gate, systemPrompt match mapping, contextPrompt per stage, saveArtifact map 13 stage. (5) Rewrite prompt `phased_master.php` (self-contained web), baru `phased_master_mobile.php`, `phases_mobile.php`. (6) `Version::defaultStageStatus()` → 13 keys. (7) `GenerateStreamController` validStages 13. (8) Frontend: `mock.ts` 13 StageKey, `new/page.tsx` colMap 13 stage + render phases_web/mobile + master_web/mobile + resume. (9) Migrations/factories/seeders: enum `web|both`, hapus `mobile`. (10) DB: truncate 5 pipeline tables, update template "Mobile CRUD" target mobile→both. (11) Sinkronisasi docs 00-19 + web/AGENTS.md.
- Perintah/hasil: `php artisan test` → 131 passed (469 assertions). `tsc --noEmit` → 0 errors. `eslint` → 0 errors (5 warning pre-existing). Pint → 5 style fix.
- Commit: `820f89a feat: pipeline 13-stage overhaul (web+mobile split, gate, mobile target removed)`
- Status: [x] Semua checkpoint A–I selesai. Docs sync selesai.
...
- **P8 — GlitchTip Integration:**
  - **Infrastructure:** `docker-compose.yml` tambah service `glitchtip` (image `glitchtip/glitchtip:6`, `SERVER_ROLE: all_in_one`, expose internal `8000`). Reuse existing `aiplanstudio_db` (PostgreSQL DB `glitchtip`, dibuat via `psql`) + `aiplanstudio_redis` (DB index 2, password via `${REDIS_PASSWORD}`). Volume `glitchtip-uploads`. `GLITCHTIP_SECRET_KEY` di root `.env` (generated `openssl rand -hex 32`). Always-on (no profile).
...
- **P7 — RS-9 FPM:** Terverifikasi **sudah terimplementasi** di code (`api/Dockerfile` CMD `php-fpm -F`, expose 9000; service `aiplanstudionginx_api` nginx listen 8000 → `fastcgi_pass api-fpm:9000`). Hanya docs yang basi → sinkron: 16-audit-fix-plan (RS-9 ✅, sisa 0), 17-next-progress (P7 `[x]`), 02-architecture (diagram + service table + note), 07-docker-setup (Dockerfile, topologi, env note).
...
- Dikerjakan: Buat 10 migration files (users dengan role, ai_providers, templates, projects, versions, phase_progress + Laravel defaults). Install Sanctum (`personal_access_tokens`). Jalankan migrasi di container `aiplanstudio_db`. Buat DatabaseSeeder dengan admin user (admin@aistack.dev / password123), AI Provider preset (empty api_key), dan 3 templates (SaaS Dashboard, E-Commerce, Mobile CRUD).
