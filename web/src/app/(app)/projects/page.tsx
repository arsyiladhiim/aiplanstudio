...
function ProjectsContent() {
  ...
  const [targetFilter, setTargetFilter] = useState<"all" | "web" | "both">(
    "all"
  )
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) {
          ...
          setPage(1)
          setHasMore(
            data.current_page != null && data.last_page != null
  ...
  async function loadMore() {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Gagal memuat lebih banyak project"
      )
    } finally {
  ...
  async function togglePin(projectId: number) {
    ...
    setProjects((list) =>
      list.map((p) =>
        p.id === projectId ? { ...p, is_pinned: nextPinned } : p
      )
    )
    try {
      ...
      setProjects((list) =>
        list.map((p) =>
          p.id === projectId ? { ...p, is_pinned: prev.is_pinned } : p
        )
      )
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={
          archivedOnly
            ? "Project yang diarsipkan. Klik untuk membuka."
            : "Semua plan yang kamu buat, lengkap dengan versi & progress."
        }
        action={
          <ButtonLink href="/new">
      ...
      <div className="mb-4 flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] py-2.5 pr-4 pl-10 text-sm"
          />
        </div>
        <button
        ...
        <select
          value={targetFilter}
          onChange={(e) =>
            setTargetFilter(e.target.value as "all" | "web" | "both")
          }
                  ...
                  <button
                    ...
                    className={`absolute top-3 right-3 inline-flex h-7 w-7 items-center justify-center rounded-lg transition ${
                      p.is_pinned
                        ...
                        : "text-[var(--color-fg-muted)] opacity-0 group-hover:opacity-100 hover:bg-[var(--color-surface-2)] hover:text-amber-400"
                    }`}
                  >
                    <Pin
                      size={14}
                      fill={p.is_pinned ? "currentColor" : "none"}
                    />
                  </button>
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[var(--color-surface-2)]">
                        <div
                          ...
                          style={{
                            width: `${((p.progress ?? 0) / getStages(p.target).length) * 100}%`,
                          }}
                        />
                      </div>
                        ...
                        p.latest_version_id && (
                          <Button
                            ...
                            onClick={(e) => {
                              e.preventDefault()
                              router.push(
                                `/new?resume=1&version=${p.latest_version_id}`
                              )
                            }}
                            className="shrink-0"
                          >
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    {p.archived_at ? (
                          ...
                          try {
                            ...
                            setError(
                              err instanceof Error
                                ? err.message
                                : "Gagal membatalkan arsip"
                            )
                          }
                        }}
                        className="inline-flex items-center gap-1 text-xs text-amber-500 transition hover:text-amber-400"
                        title="Batal arsip"
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-xs text-[var(--color-fg-muted)] transition hover:text-red-500"
                        title="Hapus project"
                      >
