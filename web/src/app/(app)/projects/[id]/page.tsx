...
export default function ProjectDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const stageStatus = (selectedVersion?.stage_status ?? {}) as Record<string, string>;
    ...
    const pp = createPhaseProgressStream(
      ...
      (event) => {
        if (event === "phase_progress" || event === "done") {
          apiGet<Version>(`/versions/${selectedVersion.id}`).then(v => {
            const newStages = getStages(v.project?.target ?? "web");
            allDoneRef.current = newStages.every(s => (v.stage_status as Record<string, string>)?.[s.key] === "done");
  ...
  return (
    <ErrorBoundary>
        ...
        <div className="flex shrink-0 items-center gap-2">
          <button
            onClick={async () => {
              ...
              setProject(prev => prev ? { ...prev, is_favorite: !prevFav } : null);
              try {
                ...
                setProject(prev => prev ? { ...prev, is_favorite: prevFav } : null);
                setError(err instanceof Error ? err.message : 'Gagal mengubah favorit');
              }
            }}
          ...
          <button
            onClick={async () => {
              ...
              setProject(prev => prev ? { ...prev, archived_at: prevArchive ? null : new Date().toISOString() } : null);
              try {
                const res = await apiPatch<{ archived_at: string | null }>(`/projects/${id}/archive`);
                setProject(prev => prev ? { ...prev, archived_at: res.archived_at } : null);
              } catch (err) {
                setProject(prev => prev ? { ...prev, archived_at: prevArchive } : null);
                setError(err instanceof Error ? err.message : 'Gagal mengubah arsip');
              }
            }}
            className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm transition ${
              project.archived_at ? "text-amber-500" : "text-[var(--color-fg-muted)] hover:text-amber-400"
            }`}
          ...
          <Button variant="secondary" size="sm" onClick={() => setConfirmDeleteProject(true)}>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        ...
        {versions.map((v) => (
          <div key={v.id} className="inline-flex items-center gap-0.5">
            ...
            {versions.length > 1 && (
              <button
                onClick={(e) => { e.stopPropagation(); setConfirmDeleteVersionId(v.id); }}
      ...
      {!versionLoading && selectedVersion && (
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_300px]">
          ...
          <Card className="overflow-hidden p-0">
            <div className="flex border-b border-[var(--color-border)]" role="tablist" aria-label="Artefak versi">
              {TABS.filter((t) => t.key !== "mobile" || project?.target === "both").map((t) => (
                <button
                  key={t.key}
              ...
              <button
                onClick={() => {
                  const content = tab === 'overview' ? JSON.stringify(selectedVersion, null, 2) :
                    tab === 'web' ? (selectedVersion.master_prompt ?? '') :
                    tab === 'mobile' ? (selectedVersion.mobile_master_prompt ?? '') :
                    tab === 'tracking' ? JSON.stringify(selectedVersion.phase_progress, null, 2) : '';
            ...
            <div className="p-5 max-h-[600px] overflow-auto" role="tabpanel">
              {tab === "overview" && (
                <div className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="rounded-lg border border-[var(--color-border)] p-4">
                      <h4 className="mb-2 text-sm font-semibold">Info Proyek</h4>
                      <dl className="space-y-1 text-sm text-[var(--color-fg-muted)]">
                        <dt className="inline font-medium text-[var(--color-fg)]">Target: </dt>
                        <dd className="inline">{project?.target === "both" ? "Web + Mobile" : "Web"}</dd>
                        <br />
                        <dt className="inline font-medium text-[var(--color-fg)]">Versi: </dt>
                        <dd className="inline">v{selectedVersion.version_no}</dd>
                        <br />
                        <dt className="inline font-medium text-[var(--color-fg)]">Pipeline: </dt>
                        <dd className="inline">{doneStages}/{totalStages} tahap</dd>
                      </dl>
                    </div>
                    <div className="rounded-lg border border-[var(--color-border)] p-4">
                      <h4 className="mb-2 text-sm font-semibold">Klarifikasi</h4>
                      {!selectedVersion.answers || Object.keys(selectedVersion.answers).length === 0 ? (
                        <p className="text-xs text-[var(--color-fg-subtle)]">Belum ada data klarifikasi.</p>
                      ) : (
                        <div className="space-y-2">
                          {Object.entries(selectedVersion.answers as Record<string, string>).slice(0, 4).map(([q, a]) => (
                            <div key={q}>
                              <p className="text-xs font-medium">{q}</p>
                              <p className="text-xs text-[var(--color-fg-muted)]">{a.length > 80 ? a.slice(0, 80) + "…" : a}</p>
                            </div>
                          ))}
                        </div>
                      )}
                  ...
                  <div className="rounded-lg border border-[var(--color-border)] p-4">
                    ...
                    <p className="text-sm text-[var(--color-fg-muted)]">{project?.idea ?? "—"}</p>
                  </div>
                </div>
              )}
              {tab === "web" && (
                  ...
                  {selectedVersion.master_prompt && (
                    <Card className="p-4">
                      <div className="mb-3 flex items-center justify-between">
                        ...
                        <Button variant="secondary" size="sm" onClick={() => copyToClipboard(selectedVersion.master_prompt as string)}>
                          <Copy size={13} /> Salin
                        </Button>
                      </div>
                      <Markdown className="text-sm leading-relaxed text-[var(--color-fg-muted)]">{selectedVersion.master_prompt}</Markdown>
                    </Card>
                  )}
                </div>
              )}
              ...
              {tab === "tracking" && (
                <div>
                  {(phases.length === 0 && mobilePhases.length === 0) ? (
                    <p className="text-[var(--color-fg-subtle)]">Belum ada fase. Jalankan pipeline sampai Master Prompt untuk generate tracking.</p>
                  ) : (
                    <TrackingPanel
                    ...
                    {selectedVersion.standards ? (
                      <div className="flex gap-2">
                        ...
                        <Button variant="secondary" size="sm" disabled={regeneratingWeb} onClick={async () => {
                          ...
                          finally { setRegeneratingWeb(false); }
                        }}>
                          {regeneratingWeb ? <Loader2 size={13} className="animate-spin" /> : <RotateCcw size={13} />} Regenerate
                        </Button>
                      </div>
                    ) : (
                      <Button variant="secondary" size="sm" disabled={regeneratingWeb} onClick={async () => {
                        ...
                        {regeneratingWeb ? <Loader2 size={13} className="animate-spin" /> : <Copy size={13} />} Generate
                      </Button>
                    )}
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    {selectedVersion.agents ? (
                      ...
                      <span className="text-xs text-[var(--color-fg-subtle)]">Klik Generate di atas (regenerasi Standards + AGENTS sekaligus)</span>
                    )}
                  </div>
                        ...
                        {selectedVersion.mobile_standards ? (
                          <div className="flex gap-2">
                            ...
                            <Button variant="secondary" size="sm" disabled={regeneratingMobile} onClick={async () => {
                              ...
                              finally { setRegeneratingMobile(false); }
                            }}>
                              {regeneratingMobile ? <Loader2 size={13} className="animate-spin" /> : <RotateCcw size={13} />} Regenerate
                            </Button>
                          </div>
                        ) : (
                          <Button variant="secondary" size="sm" disabled={regeneratingMobile} onClick={async () => {
                            ...
                            {regeneratingMobile ? <Loader2 size={13} className="animate-spin" /> : <Copy size={13} />} Generate
                          </Button>
                        )}
                      </div>
                      <div className="flex items-center justify-between">
                  ...
                  {(selectedVersion.stage_status as Record<string, string>).analisa === 'done' && (
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await apiPost(`/versions/${selectedVersion.id}/restart-from-analisa`);
                          fetchVersion(selectedVersion.id);
                        } catch (err) {
                          setError(err instanceof Error ? err.message : 'Gagal restart dari analisa');
                        } finally {
                    ...
                    >
                      {restartingAnalisa ? <Loader2 size={13} className="animate-spin" /> : <RotateCcw size={13} />}
                    </Button>
                  )}
                </div>
                <div className="space-y-3">
      ...
      <Modal open={showVersionDialog && !!project} onClose={() => setShowVersionDialog(false)} title="Buat Versi Baru" size="sm">
        <div className="space-y-4">
          <div>
      ...
      <Modal open={editingProject && !!project} onClose={() => setEditingProject(false)} title="Edit Project" size="md">
        <div className="space-y-4">
          <div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => confirmDeleteVersionId !== null && handleDeleteVersion(confirmDeleteVersionId)}
        title="Hapus Versi?"
