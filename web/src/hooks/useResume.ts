...
export interface VersionLite {
  id: number;
  project?: { id: number; title?: string; idea?: string; target?: Target } | null;
  ...
  phase_progress?: Array<{ phase_key: string; done?: boolean; status?: StageState }> | null;
...
export function useResume(resumeVersionId: number | null, isResume: boolean, alreadyStarted: boolean) {
    ...
    apiGet<VersionLite>(`/versions/${resumeVersionId}`)
      .then((v) => {
        ...
        let firstIdx = resumeStages.findIndex(
          (s) => (v.stage_status as Record<string, StageState> | undefined)?.[s.key] !== "done"
        );
        ...
        const loadedStatus: Record<StageKey, StageState> = Object.fromEntries(
          resumeStages.map((s) => [
            s.key,
            ((v.stage_status as Record<string, StageState> | undefined)?.[s.key] || "pending") as StageState,
          ])
        ) as Record<StageKey, StageState>;
        resumeStages.forEach((s) => {
          if (loadedStatus[s.key] === "error" || loadedStatus[s.key] === "running") {
        ...
        resumeStages.forEach((s) => {
          ...
          const val = v[col];
          if (val) {
            loadedArtifacts[s.key] = typeof val === "object" ? JSON.stringify(val) : String(val);
          }
        });
        ...
        const phasesRaw = String((loadedArtifacts.phases_web as string | undefined) || "[]");
        ...
        if (!resumeWebTrackingDone && masterWebIdx >= 0 && idx > masterWebIdx) {
          ...
          resumeError = "Tracking fase web belum selesai. Lanjutkan hanya setelah kamu yakin web sudah jadi.";
        }
