...
const LiveProgressWidget = dynamic(
  () =>
    import("@/components/LiveProgressWidget").then((m) => ({
      default: m.LiveProgressWidget,
    })),
  { ssr: false }
)
...
const WhatsNewModal = dynamic(
  () =>
    import("@/components/WhatsNewModal").then((m) => ({
...
const nav = [
  ...
  { href: "/ideas", label: "Bank Ide", icon: Lightbulb, adminOnly: true },
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const isActive = (item: (typeof nav)[number]) =>
    pathname === item.href ||
    (item.match !== undefined && pathname.startsWith(item.match))
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="glass sticky top-0 z-30 flex items-center gap-3 border-b border-[var(--color-border)] px-4 py-3">
          ...
          >
            <Search
              size={16}
              className="absolute top-1/2 left-3 -translate-y-1/2 text-[var(--color-fg-subtle)]"
            />
            <input
              placeholder="Cari project…"
              value={searchQ}
              onChange={(e) => setSearchQ(e.target.value)}
              className="h-10 w-full max-w-xs rounded-full border border-[var(--color-border)] bg-[var(--color-bg-soft)] pr-4 pl-9 text-sm outline-none focus:border-[var(--color-brand)]"
            />
          </form>
          <div className="ml-auto flex items-center gap-2">
            ...
            <ThemeToggle />
            <ButtonLink variant="secondary" size="sm" href="/help">
              Bantuan
            </ButtonLink>
          </div>
        </header>
