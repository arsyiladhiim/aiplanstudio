import { Check, Loader2, AlertCircle, Circle, RotateCcw, SkipForward, Minus } from "lucide-react";
import type { LucideIcon } from "lucide-react";
import {
  HelpCircle, FileText, Layers, Database, Code2, Palette,
  ListChecks, ShieldCheck, Rocket, Activity, Bot,
  FileCode, BookOpen,
} from "lucide-react";
...
export function StageRow({ stageKey, label, status = "pending", quality, onRegenerate, isRegenerating = false, onSkip, isSkipping = false }: StageRowProps) {
  ...
  const statusColor =
    ...
    status === "skipped" ? "text-[var(--color-fg-subtle)]" :
    "text-[var(--color-fg-subtle)]";

  return (
    <div className="group flex items-center gap-2 text-xs transition-colors hover:bg-[var(--color-surface-2)] rounded-md px-1.5 py-1">
      ...
      <span className={`flex-1 ${status === "done" || status === "skipped" ? "text-[var(--color-fg-subtle)]" : status === "error" ? "text-[var(--color-danger)]" : ""}`}>
        {label}
        {status === "skipped" && (
      ...
      {status === "done" && onRegenerate && (
        <button
          ...
          className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-[var(--color-surface-3)]"
          title={`Regenerate ${label}`}
        >
          {isRegenerating ? (
            <Loader2 size={11} className="animate-spin text-[var(--color-brand)]" />
          ) : (
            <RotateCcw size={11} className="text-[var(--color-fg-muted)] hover:text-[var(--color-fg)]" />
          )}
        </button>
      )}
      ...
      {status !== "done" && status !== "running" && status !== "error" && onSkip && (
        ...
        >
          {isSkipping ? (
            ...
            <SkipForward size={11} className="text-[var(--color-fg-muted)] hover:text-[var(--color-fg)]" />
          )}
        </button>
      )}
    </div>
