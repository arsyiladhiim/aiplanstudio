        ...
        {(data.fields ?? []).map((f) => {
          ...
          return (
            ...
            >
              ...
              {pk && (
                <span className="ml-auto rounded bg-[var(--color-brand)]/10 px-1 text-[9px] font-bold text-[var(--color-brand)]">
                  PK
                </span>
              )}
              {fk && !pk && (
                <span className="ml-auto rounded bg-amber-500/10 px-1 text-[9px] font-bold text-amber-600">
                  FK
                </span>
              )}
            </div>
...
function layoutGraph(erd: ErdData): { nodes: Node[]; edges: Edge[] } {
  ...
  // Anti-cycle: relasi siklik (self-ref, A→B→A) valid di ERD; tanpa guard ini
  // while-loop tak pernah berhenti (tab freeze). Setiap node diproses maks 1x.
