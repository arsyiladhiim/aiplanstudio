...
class GenerateStreamTest extends TestCase
{
    ...
    public function test_requires_version_parameter(): void
    {
        $response = $this->actingAs($this->user, 'sanctum')
            ->getJson('/api/generate/stream?stage=analisa');
    ...
    {
        $response = $this->actingAs($this->user, 'sanctum')
            ->getJson("/api/generate/stream?version={$this->version->id}");
    ...
    {
        $response = $this->actingAs($this->user, 'sanctum')
            ->getJson("/api/generate/stream?version={$this->version->id}&stage=invalid");

        $response->assertStatus(422);
        $response->assertJson(['message' => 'Stage tidak valid. Pilih: analisa, prd, architecture, erd, phases, master']);
    }

    public function test_returns_streamed_response_with_correct_headers(): void
    {
        $response = $this->actingAs($this->user, 'sanctum')
            ->get("/api/generate/stream?version={$this->version->id}&stage=analisa&auto=0");
    ...
    public function test_rejects_other_users_version(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->getJson("/api/generate/stream?version={$otherVersion->id}&stage=analisa");
    ...
    {
        $response = $this->getJson("/api/generate/stream?version={$this->version->id}&stage=analisa");
